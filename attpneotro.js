...
if (Config.WORKTYPE == 'private') {
    Asena.addCommand({ pattern: 'among ?(.*)', fromMe: true, desc: Lang.TTP_DESC }, (async (message, match) => {
    ...
    Asena.addCommand({ pattern: '2attp ?(.*)', fromMe: true, desc: Lang.ATTP_DESC }, (async (message, match) => {
        if (match[1] === '') return await message.client.sendMessage(message.jid,Lang.NEED_WORD, MessageType.text);
        var uri = encodeURI(match[1])
