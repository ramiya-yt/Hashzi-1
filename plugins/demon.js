...
Asena.addCommand({pattern: 'deson ?(.*)', fromMe: true,dontAddCMDList: true }, (async (message, match) => {    
...
Asena.addCommand({pattern: 'desoff ?(.*)', fromMe: true, dontAddCMDList: true}, (async (message, match) => {    
    var im = await checkImAdmin(message);
    if (!im) return await message.client.sendMessage(message.jid,Lang.IM_NOT_ADMIN,MessageType.text);
